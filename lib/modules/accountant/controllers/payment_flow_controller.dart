 ...
 // NEW FLOW: Triggered by "Use for Payment" button
  Future<void> startUpiPaymentFlow() async {
    ...
    try {
      ...
      finalParams['pn'] = originalParams['pn'] ?? scannedDetails['pn'] ?? 'Merchant';
      ...
      try {
        final String? result = await const MethodChannel('com.enterprise.cash/upi')
      ...
      if (response == "cancelled" || response == "result_missing") {
        ...
        if (rawStatus == 'SUCCESS') {
          ...
          approvalRef = responseParams['approvalrefno'] ?? responseParams['txnref'];
        } else if (rawStatus == 'SUBMITTED') {
          status = 'pending'; // Treat as pending/success for now or wait? User wants update.
      ...
      if (status == 'cancelled') {
         ...
         _confirmPaymentResult(status: 'failure', errorMessage: "UPI Status: $status");
      }
